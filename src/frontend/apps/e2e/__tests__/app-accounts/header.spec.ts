...
test.describe('Header', () => {
  test('checks all the elements are visible', async ({ page }) => {
    ...
    await expect(header.locator('h1').getByText('My Account')).toHaveCSS(
      'font-family',
      /Roboto/i,
    );
  ...
  test('checks all the elements are visible with DSFR theme', async ({
    ...
    await expect(header.locator('h1').getByText('My Account')).toHaveClass(
      /sr-only/,
    );
  });
  ...
  test('checks a custom waffle', async ({ page }) => {
      ...
      theme_customization: {
        waffle: {
          data: {
            services: [
              {
                name: 'My Account E2E Custom 1',
                url: 'https://accounts.numerique.gouv.fr/',
                maturity: 'stable',
                logo: 'https://lasuite.numerique.gouv.fr/assets/products/docs.svg',
              },
              {
                name: 'My Account E2E Custom 2',
                url: 'https://accounts.numerique.gouv.fr/',
                maturity: 'stable',
                logo: 'https://lasuite.numerique.gouv.fr/assets/products/docs.svg',
              },
    ...
    await expect(
      page.getByRole('link', { name: 'My Account E2E Custom 1' }),
    ).toBeVisible();
    await expect(
      page.getByRole('link', { name: 'My Account E2E Custom 2' }),
    ).toBeVisible();
  });
