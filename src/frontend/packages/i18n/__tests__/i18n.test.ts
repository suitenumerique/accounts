...
describe('integration testing on i18n package', () => {
  ...
  test('cmd extract-translation:accounts', () => {
    // To be sure the file is not here
    fs.rmSync('./locales/accounts/translations-crowdin.json', {
      recursive: true,
      force: true,
    });
    expect(
      fs.existsSync('./locales/accounts/translations-crowdin.json'),
    ).toBeFalsy();

    // Generate the file
    execSync('yarn extract-translation:accounts');
    expect(
      fs.existsSync('./locales/accounts/translations-crowdin.json'),
    ).toBeTruthy();
  });
