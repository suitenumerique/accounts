...
from oidc_provider.settings import OIDCProviderSettings, PEMValue, _value_format_pem_str
...
def test_value_format_pem_str_formats_env_style_escaped_newlines():
    """Literal escaped newlines from .env files should be normalized."""
    value = r'"-----BEGIN RSA PRIVATE KEY-----\nAB CD\n-----END RSA PRIVATE KEY-----"'
...
def test_pem_value_reads_env_style_escaped_newlines(monkeypatch):
    """PEMValue should normalize .env-style escaped PEM strings from the environment."""
    monkeypatch.setenv(
        "TEST_PEM_VALUE",
...
def test_pem_value_raises_explicit_error_for_invalid_pem(monkeypatch):
    """PEMValue should preserve the explicit PEM validation error message."""
    monkeypatch.setenv("TEST_INVALID_PEM_VALUE", "not a pem value")

    with pytest.raises(ValueError, match="Invalid PEM format"):
        PEMValue(
            default=None,
            environ_name="TEST_INVALID_PEM_VALUE",
            environ_prefix=None,
        )
