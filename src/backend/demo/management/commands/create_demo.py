...
def create_demo(stdout):
    ...
    with Timeit(stdout, "Creating OAuth2 applications"):
        for app_config in defaults.OAUTH2_APPLICATIONS:
            application = Application(
                ...
                **{
                    ...
                    "skip_authorization": app_config.get("skip_authorization", False),
                },
            )
    ...
    with Timeit(stdout, "Creating users"):
        ...
        for i in range(defaults.NB_OBJECTS["users"]):
            ...
            queue.push(
                models.User(
                    username=f"user.test{i:d}@example.com",
